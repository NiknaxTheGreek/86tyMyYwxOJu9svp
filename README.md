...
![Word2Vec ranking quality](figures/readme_01_ndcg.png)
...
![Feedback-strength trade-off](figures/readme_02_feedback_tradeoff.png)
...
![Seeking-HR ranking movement after feedback](figures/readme_03_rank_movement.png)
...
The final project contains **10 core files**: the original candidate dataset, a 52-title manually labelled dataset used for the independent human-relevance analysis, three executed notebooks covering rule-based ranking, Word2Vec ranking, and recruiter-feedback reranking, the README, a Technical Report, a Business Recommendations report, `requirements.txt`, and `.gitignore`.
...
The project is implemented as a reproducible **Python workflow across three executed notebooks**. The original source data are preserved, and preprocessing rules, relevance logic, evaluation metrics, and the selected 30% feedback weight are explicitly defined in code.

The pretrained GoogleNews Word2Vec model itself is not bundled because of its size; the project documents the exact **3-million-word, 300-dimensional model specification** required to reproduce the semantic ranking.
